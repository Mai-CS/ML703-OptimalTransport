...
class Star(nn.Module):
    def __init__(
        ...
        self.star = StarTransformerClassifier(
            num_cycles, hidden_size, hidden_size // 64, dropout, n_neurons
        )
...
class Encoder(nn.Module):
    def __init__(
        self, input_size: int = 7508, hidden_size: int = 1024, num_layers: int = 3
    ) -> None:
...
class Bert(nn.Module):
    def __init__(self, pretrained_bert_dir: str = "", freeze: tuple = ()):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, embed_dim: int = 768, dropout: float = 0.1):
        ...
        self.attention = nn.MultiheadAttention(
            embed_dim=embed_dim, num_heads=embed_dim // 64, dropout=dropout
        )
...
class AUROC(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        all_preds2 = (all_preds > 0.5).astype(int)
        f1_score = metrics.f1_score(all_y, all_preds2, average="macro")
